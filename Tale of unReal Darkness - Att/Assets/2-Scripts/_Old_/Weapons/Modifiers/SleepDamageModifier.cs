...
using SleepDamageData = Bardent.Combat.SleepDamage.SleepDamageData;
...
namespace Bardent.Weapons.Modifiers
{
     ...
     */
    public class SleepDamageModifier : Modifier<SleepDamageData>
    {
        ...
        public SleepDamageModifier(ConditionalDelegate isBlocked)
        {
         ...
         */
        public override SleepDamageData ModifyValue(SleepDamageData value)
        {
            if (isBlocked(value.Source.transform, out var blockDirectionInformation))
            {
                value.SetAmount(value.Amount * (1 - blockDirectionInformation.SleepDamageAbsorption));
