...
using FireDamageData = Bardent.Combat.FireDamage.FireDamageData;
...
namespace Bardent.Weapons.Modifiers
{
     ...
     */
    public class FireDamageModifier : Modifier<FireDamageData>
    {
        ...
        public FireDamageModifier(ConditionalDelegate isBlocked)
        {
         ...
         */
        public override FireDamageData ModifyValue(FireDamageData value)
        {
            if (isBlocked(value.Source.transform, out var blockDirectionInformation))
            {
                value.SetAmount(value.Amount * (1 - blockDirectionInformation.FireDamageAbsorption));
