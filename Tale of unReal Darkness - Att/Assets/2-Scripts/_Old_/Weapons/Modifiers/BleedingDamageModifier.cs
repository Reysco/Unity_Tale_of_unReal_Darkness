...
using BleedingDamageData = Bardent.Combat.BleedingDamage.BleedingDamageData;
...
namespace Bardent.Weapons.Modifiers
{
     ...
     */
    public class BleedingDamageModifier : Modifier<BleedingDamageData>
    {
        ...
        public BleedingDamageModifier(ConditionalDelegate isBlocked)
        {
         ...
         */
        public override BleedingDamageData ModifyValue(BleedingDamageData value)
        {
            if (isBlocked(value.Source.transform, out var blockDirectionInformation))
            {
                value.SetAmount(value.Amount * (1 - blockDirectionInformation.BleedingDamageAbsorption));
